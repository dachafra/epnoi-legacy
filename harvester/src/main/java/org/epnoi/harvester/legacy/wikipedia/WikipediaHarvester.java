...
public class WikipediaHarvester {
	...
	private void _harvestWikipediaDumpsPath() {
		...
		List<File> listOfDumps = FolderUtils.listFiles(folder);

		logger.info("Harvesting the path " + folder.getAbsolutePath());

		for (File file: listOfDumps){
			_harvestWikipediaDump(file);
	...
	public static void main(String[] args) {
		...
		parameters.setParameter(WikipediaHarvesterParameters.INCREMENTAL, false);
		parameters.setParameter(WikipediaHarvesterParameters.NUMBER_OF_THREADS, 40);
